...
- **登录前置 = 策划第 0 步，而不是行程做完后的补救**：开始任何取数/策划前，先用**应用内浏览器（侧边栏）直接打开各平台页面**（携程 hotels.ctrip.com、大众点评 www.dianping.com/<城市拼音>、需要时飞猪 www.fliggy.com/jiudian/），让用户现场扫码登录；未登录时不开始取数，更不要等攻略交付后才补一句「需要登录」。
...
- 打开后**停下等待**：告诉用户「登录页已打开，请扫码登录，好了告诉我」；不要替用户点登录、不代解验证码、不在用户登录过程中反复刷新或切换页面。
- 客户完成后，把「携程/大众点评（+飞猪）已登录」记入当前任务状态；后续 Step 5/6 只复用会话、重新打开标签页，不再要求重新登录。
